...
export default function JournalTable({ trades, compact, onEdit, onDelete, showCOP = true }: JournalTableProps) {
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            {['Date', 'Asset', 'Type', 'Strategy', 'P/L (USD)', ...(showCOP ? ['COP'] : []), 'Status'].map((h) => (
        ...
        <tbody>
          {rows.map((t, i) => (
            <tr
              key={t.id}
              style={{ transition: 'background 0.15s', animation: `rowIn 0.3s ease ${i * 40}ms both` }}
            ...
            >
              ...
              {showCOP && (
                <td style={{ ...td, color: t.cop >= 0 ? 'var(--green)' : 'var(--red)', fontSize: 12 }}>
                  {t.cop >= 0 ? '+ COP ' : '-COP '}{Math.abs(t.cop).toLocaleString()}
                </td>
              )}
              <td style={td}>
                <span style={{
